document.addEventListener('DOMContentLoaded', () => {
    ...
    if (analysisData.improvementAreas?.length > 0) {
        ...
        improvementAreas.innerHTML = '<div class="text-text-secondary">No immediate improvements needed</div>';
    }
...
function updatePatientInfo(patientInfo) {
    ...
    document.getElementById('patientAge').textContent = `${patientInfo.age} years`;
    document.getElementById('patientGender').textContent = patientInfo.gender;
    document.getElementById('patientBMI').textContent = `${patientInfo.bmi} (${patientInfo.bmiCategory})`;
    document.getElementById('patientEthnicity').textContent = patientInfo.ethnicity;

    // Update conditions
    const conditionsEl = document.getElementById('patientConditions');
    if (patientInfo.conditions?.length > 0) {
        conditionsEl.innerHTML = patientInfo.conditions.map(condition => `
    ...
    if (patientInfo.medications?.length > 0) {
        medicationsEl.innerHTML = patientInfo.medications.map(medication => `
            <div class="flex items-center space-x-2">
    ...
    if (patientInfo.allergies?.length > 0) {
        allergiesEl.innerHTML = patientInfo.allergies.map(allergy => `
            <div class="flex items-center space-x-2">
...
function renderMedicineRecommendations(medicines) {
    ...
    const sortedMedicines = [...medicines].sort((a, b) => b.score - a.score);
    
    // Render top medicine
    const topMedicine = sortedMedicines[0];
    document.getElementById('topMedicine').innerHTML = `
        <div class="bg-success-light p-4 rounded-lg border border-success-color">
            <div class="flex justify-between items-start mb-3">
                <div>
                    <h4 class="text-lg font-semibold text-success-color">${topMedicine.name}</h4>
                    <p class="text-sm text-text-secondary mt-1">${topMedicine.description}</p>
                </div>
                <div class="text-success-color font-semibold">${topMedicine.score}/100</div>
            </div>
            <div class="space-y-3">
                <div>
                    <div class="text-sm font-medium text-text-primary mb-1">Recommended Dosage</div>
                    <div class="text-sm">${topMedicine.dosage}</div>
                </div>
                ${topMedicine.warnings.length > 0 ? `
                    <div>
                        <div class="text-sm font-medium text-danger-color mb-1">Warnings</div>
                ...
                ${topMedicine.interactions.length > 0 ? `
                    <div>
                        <div class="text-sm font-medium text-warning-color mb-1">Drug Interactions</div>
                        <ul class="text-sm list-disc list-inside space-y-1">
        ...
        <div class="medicine-card animate-slide-in">
            ...
            <div class="mt-3 space-y-2">
                ...
                ${medicine.warnings.length > 0 ? `
                    <div>
                        <div class="text-sm font-medium text-danger-color">Warnings</div>
...
function getMetricStatus(metric) {
    const difference = Math.abs(metric.value - metric.normalValue);
    const threshold = metric.normalValue * 0.1; // 10% threshold
    
    if (difference <= threshold) return 'normal';
    if (difference <= threshold * 2) return 'warning';
    return 'critical';
}
